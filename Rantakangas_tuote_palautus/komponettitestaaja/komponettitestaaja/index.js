...
function komponenttilista(kansio){
    ...
    const hakemistot = tiedostot.filter(tiedosto=>tiedosto.isDirectory()).map(hakemisto=>hakemisto.name);
...
function muodostaKotisivu(kansiot){
    ...
    <body>
        <main>
            <h1>Valikko</h1>`;
    for(const komponenttipolku of kansiot){
        const [kansio,nimi,tiedosto] = komponenttipolku.split('/');
